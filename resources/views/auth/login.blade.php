    ...
    <div class="col-md-4">
        <div class="login-form col-md-12">
            <div class="login-content">
                <p class="login-box-msg">@lang('lang_v1.login')</p>
                <form method="POST" action="{{ route('login') }}" id="login-form">
                    {{ csrf_field() }}
                    <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                        @php
                            $username_label = 'lang_v1.username';
                            if (session('business.enable_business_based_username') == 1) {
                                $username_label = 'lang_v1.username_ext';
                            }
                        @endphp

                        <input id="username" type="text" class="form-control" name="username" value="{{ $username }}"
                            required autofocus placeholder="@lang($username_label)">

...
use Modules\Connector\Utils\PurchaseUtil;
...
class PurchaseController extends ApiController
{
     ...
     * @return void
     */
    public function __construct(ProductUtil $productUtil, TransactionUtil $transactionUtil, BusinessUtil $businessUtil, ModuleUtil $moduleUtil ,PurchaseUtil $purchaseUtil)
    {
    ...
    {
        if (!auth('api')->user()->can('purchase.view') && !auth('api')->user()->can('purchase.create') && !auth('api')->user()->can('view_own_purchase')) {
        ...
        $permitted_locations = $this->purchaseUtil->permitted_locations_api($business_id);
        ...
        if (!auth('api')->user()->can('purchase.view') && auth('api')->user()->can('view_own_purchase')) {
            $purchases->where('transactions.created_by', $request->user_id);
        }


        $business_locations = $this->purchaseUtil->forDropdownApi($business_id);
    ...
    public function store(Request $request)
    {
        ...
        if (!auth('api')->user()->can('purchase.create')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            $enable_product_editing = $business->enable_editing_product_from_purchase ;
            ...
            //  $test = $request->input('payment');
            // return $test;
            
            //Add Purchase payments
            $this->transactionUtil->createOrUpdatePaymentLines($transaction, $request->input('payment'),$business_id,$user_id);
            // dd($purchases);
    ...
    {
        // if (!auth('api')->user()->can('purchase.view')) {
        //     abort(403, 'Unauthorized action.');
        // }
    ...
    {
        if (!auth('api')->user()->can('purchase.update')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $types = [];
        if (auth('api')->user()->can('supplier.create')) {
            $types['supplier'] = __('report.supplier');
        }
        if (auth('api')->user()->can('customer.create')) {
            $types['customer'] = __('report.customer');
        }
        if (auth('api')->user()->can('supplier.create') && auth('api')->user()->can('customer.create')) {
    ...
    {
        if (!auth('api')->user()->can('purchase.delete')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    {
        if (!auth('api')->user()->can('purchase.update') && !auth('api')->user()->can('purchase.update_status')) {
            abort(403, 'Unauthorized action.');
        }
        //Check if the transaction can be edited or not.
        $edit_days = $request->session()->get('business.transaction_edit_days');

...
class ProductUtil extends Util
{
    ...
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [])
    {
        ...
        $variation_data = [
            ...
            'sub_sku' => $sku,
            'default_purchase_price' => $this->num_uf($purchase_price,$currency_details),
            'dpp_inc_tax' => $this->num_uf($dpp_inc_tax,$currency_details),
            'profit_percent' => $this->num_uf($profit_percent,$currency_details),
            'default_sell_price' => $this->num_uf($selling_price,$currency_details),
            'sell_price_inc_tax' => $this->num_uf($selling_price_inc_tax,$currency_details),
     ...
     */
    public function generateProductSku($string ,$business_id)
    {
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            // dd($data['quantity']);
            $new_quantity = $this->num_uf($data['quantity'],$currency_details) * $multiplier;
            $new_quantity_f = $this->num_f($new_quantity,false ,$currency_details);
            // dd($new_quantity_f);
    ...
    {
        $types = ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'sell_return', 'production_sell'];
            ...
            'sl.transaction_id', '=', 'transactions.id')
                                ...
                                ->where( function($q) use ($variation_id){
                                    $q->where('sl.variation_id', $variation_id)
                                        ...
                                        // ->orWhere('inv_p.variation_id', $variation_id)
                                        ->orWhere('al.variation_id', $variation_id)
                                        ->orWhere('rpl.variation_id', $variation_id)
                                        ->orWhere('rsl.variation_id', $variation_id);
                                });

                                if(Schema::hasTable('emptying_inventory_products')){
                                    
                                    $types[] = 'empty';
                                    $stock_history->leftjoin('emptying_inventory_products as empty_inv_p', 
                                    'empty_inv_p.transaction_id', '=', 'transactions.id')
                                    ->orWhere(function($q) use ($variation_id,$location_id){
                                        $q->where('empty_inv_p.variation_id',$variation_id)
                                        ->where('transactions.location_id', $location_id);
                                    })
                                    ->addSelect('empty_inv_p.qty_before as empty_inventory_product_qty');

                                }

                                if(Schema::hasTable('inventory_products')){
                                    $types[] = 'inventory';
                                    $stock_history->leftjoin('inventory_products as inv_p', 
                                    'inv_p.transaction_id', '=', 'transactions.id')
                                    ->orWhere(function($q) use ($variation_id,$location_id){
                                        $q->where('inv_p.variation_id',$variation_id)
                                        ->where('transactions.location_id', $location_id);
                                    })
                                    ->addSelect('inv_p.inventory_type as inventory_type',
                                    'inv_p.amount_after_inventory as inventory_amount',
                                    'inv_p.Amount_difference as inventory_diff');
                                }

                                $stock_history->whereIn('transactions.type', $types)
                                ->addSelect(
        ...
        foreach ($stock_history as $stock_line) {
            ...
            elseif ($stock_line->transaction_type == 'empty' ) {
                

                $quantity_change = (
                        $stock_line->type == 'inc' 
                        ?   
                        abs($stock_line->empty_inventory_product_qty) 
                        : 
                        (-1 * $stock_line->empty_inventory_product_qty) 
                    );
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'empty',
                    'type_label' => __('inventoryemptying::app.empting_inventory'),
                    'ref_no' => $stock_line->ref_no ?? '',
                    'additional_notes' => $stock_line->additional_notes,
                    'purchase_secondary_unit_quantity' => !empty($stock_line->purchase_secondary_unit_quantity) ? $this->roundQuantity($stock_line->purchase_secondary_unit_quantity) : 0,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit)
            ...
            elseif (
                $stock_line->transaction_type == 'inventory' 
                && $stock_line->inventory_type == 'inventory_increase'
                ) {
                
                // if($stock_line->inventory_type == 'inventory_incremet')
                $quantity_change = $stock_line->inventory_diff;
                $stock += $quantity_change;
                $stock_in_second_unit += $stock_line->purchase_secondary_unit_quantity;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'inventory',
                    'type_label' => __('invetorymangament::inventory.increase_inv'),
                    'ref_no' => $stock_line->ref_no ?? '',
                    'additional_notes' => $stock_line->additional_notes,
                    'purchase_secondary_unit_quantity' => !empty($stock_line->purchase_secondary_unit_quantity) ? $this->roundQuantity($stock_line->purchase_secondary_unit_quantity) : 0,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit)
                ]);
            } elseif ($stock_line->transaction_type == 'inventory' && $stock_line->inventory_type == 'inventory_decrease') {
                $quantity_change = ($stock_line->inventory_diff < 0 ?   $stock_line->inventory_diff : (-1 * $stock_line->inventory_diff) );
                $stock += $quantity_change;
                $stock_in_second_unit += $stock_line->purchase_secondary_unit_quantity;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'inventory',
                    'type_label' => __('invetorymangament::inventory.decrease_inv'),
                    'ref_no' => $stock_line->ref_no ?? '',
                    'additional_notes' => $stock_line->additional_notes,
                    // 'purchase_secondary_unit_quantity' => !empty($stock_line->purchase_secondary_unit_quantity) ? $this->roundQuantity($stock_line->purchase_secondary_unit_quantity) : 0,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit)

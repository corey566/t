...
@section('title', 'One Gallface Invoice History')
    ...
    <div class="gallface-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h1 style="font-size: 24px; font-weight: 700; margin: 0;">
                <i class="fas fa-file-invoice"></i> One Gallface Invoice History
            </h1>
            <a href="{{ url('gallface/setting') }}" class="btn-back">
            ...
            <table class="invoice-table" id="invoiceTable">
                <thead>
                    <tr>
                        ...
                        <th data-sort="gallface_synced_at">Synced At <i class="fas fa-sort"></i></th>
                    </tr>
                </thead>
                <tbody id="tableBody">
                    <tr>
                        <td colspan="8" class="no-data" style="text-align: center; padding: 40px;">Loading...</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
function loadInvoices() {
    ...
    $('#tableBody').html('<tr><td colspan="8" style="text-align: center; padding: 40px;">Loading...</td></tr>');
    
    $.ajax({
        url: '{{ url("gallface/location/" . $location_id . "/invoice-history") }}',
        type: 'GET',
        data: {
            date_from: dateFrom,
            date_to: dateTo,
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#tableBody').html('<tr><td colspan="8" style="text-align: center; padding: 40px;">Failed to load data</td></tr>');
            }
        },
        error: function() {
            $('#tableBody').html('<tr><td colspan="8" style="text-align: center; padding: 40px;">Error loading data</td></tr>');
        }
    });
}
...
function renderTable() {
    ...
    if (pageData.length === 0) {
        $('#tableBody').html('<tr><td colspan="8" style="text-align: center; padding: 40px;"><i class="fas fa-inbox fa-3x text-muted" style="display: block; margin-bottom: 15px;"></i><p class="text-muted">No invoices found for the selected criteria.</p></td></tr>');
    ...
    pageData.forEach(invoice => {
        const statusBadge = invoice.gallface_synced_at 
        ...
        const syncedAt = invoice.gallface_synced_at 
            ? new Date(invoice.gallface_synced_at).toLocaleString()
            : '-';
